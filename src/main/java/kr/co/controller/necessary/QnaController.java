...
public class QnaController {
	...
	public String read(@PathVariable("qna_id") String qna_id, @PathVariable("qna_num") int qna_num, Model model,
		...
		AdminVO adlogin = (AdminVO) session.getAttribute("admin");
		System.out.println(adlogin);
		if (login == null && adlogin == null) {
			System.out.println("로그인 or admin null");
			return "redirect:/member/loginGet";
		}
		if (login != null && adlogin == null) {
			System.out.println("사용자 login");
			if (!login.getM_id().equals(qna_id)) {
				System.out.println("로그인 아이디랑 접속아이디랑 다름");
				return "redirect:/qna/list";
			}
		}
	...
	public String update(@PathVariable("qna_id") String qna_id, @PathVariable("qna_num") int qna_num, Model model,
			HttpSession session) {
		MemberDTO login = (MemberDTO) session.getAttribute("login");
		AdminVO adlogin = (AdminVO) session.getAttribute("admin");
		if (login == null && adlogin == null) {
	...
	// delete.GET
	   @RequestMapping(value = "/delete/{qna_id}/{qna_num}", method = RequestMethod.GET)
	   public String delete(@PathVariable("qna_id") String qna_id, @PathVariable("qna_num") int qna_num, HttpSession session) {
	      ...
	      qService.delete(qna_id, qna_num);
	 ...
	 * void login(LoginVO vo, Model model) {
	 * 
	 * QnaDTO dto = (QnaDTO) qService.login(vo); if (dto == null)
