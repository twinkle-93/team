...
public class MemberController {
	...
	public String read(@PathVariable("m_id") String m_id, Model model, HttpSession session) {
		...
		// 두조건
		// 로그인한 아이디와 회원 아이디가 맞지 않고, 관리자가 null 이라면
		if (login != null) {
	...
	public void pointUpdate(@PathVariable("m_id") String m_id, @PathVariable("m_point") int m_point) {
		mService.pointUpdate(m_id, m_point);
	}
	//m_point read
	@RequestMapping(value = "/pointRead/{m_id}", method = RequestMethod.GET)
	public int pointUpdate(@PathVariable("m_id") String m_id) {
		return mService.pointRead(m_id);
	}
	//m_money read
	@RequestMapping(value = "/moneyRead/{m_id}", method = RequestMethod.GET)
	public int moneyRead(@PathVariable("m_id") String m_id) {
		return mService.moneyRead(m_id);
	}
	//m_money update
	@RequestMapping(value = "/moneyUpdate/{m_id}/{m_money}", method = RequestMethod.GET)
